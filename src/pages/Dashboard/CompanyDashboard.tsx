...
import { Plus, Users, Eye, MessageCircle, Edit, Trash2, Building2, Calendar, MapPin, DollarSign, AlertTriangle } from 'lucide-react';
...
export default function CompanyDashboard() {
  ...
  const handleConfirmDelete = () => {
    if (selectedJobId) {
      deleteJob(selectedJobId); // your existing function
      handleCloseModal();
    }
  };
            ...
            <nav className="flex space-x-8 px-8">
              {tabs.map((tab) => (
                ...
                >
                  ...
                  <span>{tab.label}</span>
                  <span className={`ml-2 py-1 px-2 rounded-full text-xs ${activeTab === tab.id ? 'bg-blue-100 text-blue-600' : 'bg-gray-100 text-gray-600'
                    }`}>
                    {tab.count}
                  </span>
                </button>
